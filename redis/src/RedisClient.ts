...
export class RedisSubscriptionManager {
    ...
    private constructor() {
        ...
        this.subscriptions = new Map<string, string[]>(); // userID,rooms as one user can subscribe to many rooms
        this.reverseSubscriptions = new Map<string, { [userId: string]: { userId: string; ws: any; } }>(); //given a room what all users are connected
    }
    ...
    subscribe(userId: string, room: string, ws: any) {
        this.subscriptions.set(userId, [
            ...(this.subscriptions.get(userId) || []), // check closely we add to map if not empty
            room,
        ]);
